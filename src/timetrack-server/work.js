...
exports.delete = (connection, req, res) => {
  exports.parseReceiveData(req, work => {
    connection.query('delete from work where id=?', [work.id], err => {
      if (err) throw err;
      exports.show(connection, res);
    });
  });
...
exports.archive = (connection, req, res) => {
  exports.parseReceiveData(req, work => {
    connection.query('update work set archived=1 where id=?', [work.id], err => {
      if (err) throw err;
      exports.show(connection, res);
    });
  });
...
exports.show = (connection, res, showArchived) => {
  ...
  connection.query('select * from work where archived=? order by date desc', [archiveValue], (err, rows) => {
    if (err) throw err;
